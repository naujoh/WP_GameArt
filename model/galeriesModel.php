<?php
	...
	class galeriesModel extends gameArt{
		...
		public function __construct(){
			parent::__construct();	
			$this->querys = array(
					"recents"=> "SELECT p.id_publicacion, u.id_usuario, u.nombre_usuario, u.imagen_perfil, p.titulo, r.nombre_recurso 
						FROM publicacion p 
						JOIN usuario u ON p.id_usuario=u.id_usuario 
						JOIN recurso r ON r.id_publicacion = p.id_publicacion 
						JOIN categoria c ON c.id_categoria = p.id_categoria
						WHERE c.id_tipo_publicacion = 1 AND r.id_tipo_recurso = 6
						ORDER BY p.id_publicacion DESC LIMIT 8;",

					"dropdown"=> "SELECT * FROM categoria WHERE id_tipo_publicacion = 1 ORDER BY id_categoria ASC",

					"dd_access"=>"SELECT * FROM acceso ORDER BY id_acceso ASC",

					"dd_access_selected"=>"SELECT id_acceso as id, acceso as option FROM acceso ORDER BY id_acceso ASC",
					
					"dd_selected" => 
						"SELECT id_categoria as id, categoria as option FROM categoria WHERE id_tipo_publicacion = 1 ORDER BY id_categoria ASC",					

					"all"=>"SELECT p.id_publicacion, r.nombre_recurso FROM publicacion p JOIN categoria c ON p.id_categoria=c.id_categoria JOIN recurso r ON r.id_publicacion=p.id_publicacion JOIN tipo_recurso tr ON tr.id_tipo_recurso = r.id_tipo_recurso WHERE c.id_tipo_publicacion = 1 AND tr.id_tipo_recurso=6",

					"post_content"=>
					"SELECT * FROM publicacion p JOIN categoria c ON p.id_categoria=c.id_categoria JOIN recurso r ON p.id_publicacion = r.id_publicacion JOIN usuario u ON u.id_usuario = p.id_usuario JOIN tipo_recurso tr ON tr.id_tipo_recurso=r.id_tipo_recurso WHERE p.id_publicacion=:id_publicacion AND tr.id_tipo_recurso = 4;",
					"post_comments"=>
					"SELECT * FROM comentario c JOIN usuario u ON c.id_usuario=u.id_usuario WHERE id_publicacion=:id_publicacion",
					"last_pub"=>
					"SELECT MAX(id_publicacion) as id_publicacion from publicacion WHERE id_usuario=:id_usuario;",

					"resources_tab"=>
					"SELECT * FROM recurso WHERE id_publicacion=:id_publicacion",

					"table_data" => "SELECT r.nombre_recurso, p.id_publicacion, p.titulo, p.fecha_subida, a.acceso FROM publicacion p JOIN usuario u ON u.id_usuario = p.id_usuario JOIN recurso r ON r.id_publicacion = p.id_publicacion JOIN acceso a on a.id_acceso = p.id_acceso JOIN categoria c ON c.id_categoria = p.id_categoria WHERE c.id_tipo_publicacion = 1 AND r.id_tipo_recurso = 6 AND u.id_usuario = :id_usuario;",
		...
		public function insertGalery(){
			...
			// echo '</pre>';
			// die();
			$images = $_FILES['galeria']['name'];
			$uploaded_files = $this->multiUploads($images, 'image');
			if($uploaded_files['uploaded']){
				...
				if(isset($_FILES['portada']['name'])){
					...
					if($this->checkTypeOfFile($_FILES['portada'],'image')){
						if(move_uploaded_file($source, $destination)){
							...
							$params['nombre_recurso'] = 'gallery_default.png';
							$this->insertar('recurso', $params);								
				...
				foreach ($resource_data as $key => $value) {
					if($resource_data[$key]['id_tipo_recurso'] == 4){
					//Actualizar nombre de los recursos en la BD
						$new_name['nombre_recurso'] = 'ga_'.$res_params['id_publicacion'].$resource_data[$key]['nombre_recurso'];
					...
					//Actualizar nombre de los archivos fisicos
						...
						rename('view/resources/images/post_images/'.$old_name, 'view/resources/images/post_images/'.$new_name);
						$new_name = array();
						$keys = array();
					}
				}	
