		...
		public function __construct(){
			...
			$this->querys = array(
					...
					FROM publicacion p 
						...
						WHERE c.id_tipo_publicacion = 1
						ORDER BY p.fecha_subida DESC LIMIT 6;",
					"dropdown"=> "	SELECT * FROM categoria WHERE id_tipo_publicacion = 1 ORDER BY id_categoria ASC",
					"all"=>"SELECT p.id_publicacion, r.nombre_recurso FROM publicacion p JOIN categoria c ON p.id_categoria=c.id_categoria JOIN recurso r ON r.id_publicacion=p.id_publicacion WHERE c.id_tipo_publicacion = 1"
			);
		}
